<style type="text/css">
.hh{
	background-image: url(login/images/bg_43.jpg);
	width:634px;
	height:446px;
}
</style>
...
if ($_POST[go]=="login")
{
   ...
   } else 
   {
      ...
      echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" />";
	  echo "<script>location.href='./wddq.php'</script>";
	  exit;
   }
}
...
if($_POST['go']=="register")
{
  ...
  if( mysql_query("INSERT INTO account (password,name,createtime,money,money2,jifen,gold,tg,tg2,namemd5id,regip,lastip) VALUES ('".md5($password)."','".$name."',NOW(),".$zhucerow[money].",".$zhucerow[money2].",".$zhucerow[jifen].",".$zhucerow[gold].",".$zhucerow[tg].",".$zhucerow[tg2].",'".$MD5id."','".$ip."','".$ip."')")==true)
  {
   ...
   echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" />";
   echo "<script>alert('已经成功注册成为会员！');location.href='./wddq.php'</script>";
   exit;
  }else
  {
   echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" />";
   echo "<script>alert('注册失败！SQL没有成功执行！');location.href='./wddq.php'</script>";
   exit;
  }
}
