...
if($mf_num<$chajian_row['canshu1'])
	{
		...
		$role_account_row = mysql_fetch_array(mysql_query("SELECT user_guid,lingshi FROM dntg.user_account WHERE user_guid='".$user_role_row['user_id']."'"));

		$nowtq = $role_account_row[lingshi] + $chajian_row['canshu5'] ;
		//铜钱修改
		//$nowyb = $role_account_row[bind_lingshi] + $chajian_row['canshu6'] ;
		
		$sql="UPDATE dntg.user_account SET lingshi = '".$nowtq."'  WHERE user_guid = '".$user_role_row[user_id]."';";
