...
if($mf_num<$chajian_row['canshu1'])
	{
		...
		$role_guangyi_info=mysql_query("SELECT * FROM role_guangyi WHERE user_role_id = '".$user_role_row[id]."'");
		$role_guangyi_num=mysql_num_rows($role_guangyi_info);
		$time=time()."000";
		if($role_guangyi_num==0)
		{
		   $sql="INSERT INTO role_guangyi VALUES ('".$user_role_row[id]."', '".$chajian_row[canshu5]."', '0', '0', '".$time."', '', '".$time."', '', '0', '', '2014-03-17 22:25:56');";
		}else
		{
		  $sql="UPDATE role_guangyi set binqi_level='".$chajian_row[canshu5]."' WHERE user_role_id ='".$user_role_row[id]."'";
		}
