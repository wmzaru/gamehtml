...
if($mf_num<$chajian_row['canshu1'])
	{
		...
		$role_account_row = mysql_fetch_array(mysql_query("SELECT * FROM role_account WHERE user_role_id='".$user_role_row['id']."'"));

		$nowtq = $role_account_row[tongqian] + $chajian_row['canshu5'] ;
		//铜钱修改
		$nowyb = $role_account_row[bind_lingshi] + $chajian_row['canshu6'] ;
		
		$sql="UPDATE role_account SET tongqian = '".$nowtq."' ,bind_lingshi = '".$nowyb."' WHERE user_role_id = '".$user_role_row[id]."';";
