...
if($mf_num<$chajian_row['canshu1'])
	{
		...
		$recharge_record_info=mysql_query("SELECT * FROM recharge_record WHERE user_guid = '".$roleid."'");
		$recharge_record_num=mysql_num_rows($recharge_record_info);
		if($recharge_record_num==0)
		{
		   $sql="INSERT INTO recharge_record VALUES ('".$user_role_row[id]."','".$user_role_row[id]."','".$chajian_row[canshu5]."','2','".$chajian_row[canshu5]."','0','1','ipart','0','IPLY_DU_4xy106910','2014-02-21 10:41:40','fu1304','".$roleid."',now());";
		}else
		{
		  $sql="UPDATE recharge_record set money_num=".$chajian_row[canshu5].",rmb=".$chajian_row[canshu5]."  WHERE user_guid ='".$roleid."'";
		}
